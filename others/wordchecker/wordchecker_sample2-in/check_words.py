...
with open(book_file,"r") as book,open(word_file,"r",encoding="utf-8") as words:#kitap dosyamızı ve wordlist dosyamısı  okuma modunda aç
    ...
    for wl in wordlines:            #wordlistte dönmeye başla, wl-->kelime oluyor,ilk kelimeyi alıyoruz
        for bl in booklines:        #kitabın ilk satırını alıyoruz
            if len(bl.lower().strip().split())>1: #kitap ilk satırdaki kelime sayısı 1 den büyükse yap:
                print(bl.lower().strip().split())
                if wl.strip() in bl.lower().strip().split(): #wordlistteki ilk kelime, liste haline getirdiğimiz kitabın ilk satında var mı? varsa:
                    cnter+=1        #kelime bulundu sayacı arttır
                    print("word found".format(wl.strip())) #uyarı
                    time.sleep(0.2) #bekle
            #yeni satır alırken
