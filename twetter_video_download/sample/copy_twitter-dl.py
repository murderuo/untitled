...
class TwitterDownloader:
    ...
    **Disclaimer** I wrote this to recover a video for which the original was lost. Consider copyright before downloading
    ...
    def download(self):
        ...
        token=self.__get_bearer_token() ##Bearer AAAAAAAAAAAAAAAAAAAAAIK1zgAAAAAA2tUWuhGZ2JceoId5GwYWU5GspY4%3DUq7gzFoCZs1QfwGoVdvSac3IniczZEYXIcDyumCauIXpcAPorE

        # Get the M3u8 file - this is where rate limiting has been happening
        video_host,playlist=self.__get_playlist(token)
        # print(playlist.is_variant)
                ...
                with open(str(ts_full_file),'wb') as wfd:
                    for f in ts_list:
                        with open(f,'rb') as fd:
                            shutil.copyfileobj(fd,wfd,1024*1024*10)  # for ts in ts_full_file_list:

                for ts in ts_full_file_list:
                    print('\t[*] Doing the magic ...')

                    (ffmpeg.input(ts).output(str(resolution_file), acodec = 'copy', vcodec = 'libx264', format = 'mp4', loglevel = 'error').overwrite_output().run())
    ...
    def __get_bearer_token(self):
        video_player_url=self.video_player_prefix+self.tweet_data[
            'id']  # https://twitter.com/i/videos/tweet/ + 1209143227405807617
        video_player_response=self.requests.get(video_player_url).text  ##return html code

        self.__debug('Video Player Body','',video_player_response)  # return

        js_file_url=re.findall('src="(.*js)',video_player_response)[
            0]  ## https://abs.twimg.com/web-video-player/TwitterVideoPlayerIframe.1c7d5c71349918c3.js
    ...
    def __get_playlist(self,token):
        player_config_req=self.requests.get(self.video_api+self.tweet_data['id']+'.json')##https://api.twitter.com/1.1/videos/tweet/config/1209143227405807617.json
        print(player_config_req.text)
        ...
        # print(m3u8_url_parse)
        # print(m3u8_url_parse.scheme)
        # print(m3u8_url_parse.hostname)
        # print(m3u8_url_parse.path)
...
if __name__ == '__main__':
    ...
    twitter_dl=TwitterDownloader('https://twitter.com/i/status/1209143227405807617','./output',0)
    twitter_dl.download()
