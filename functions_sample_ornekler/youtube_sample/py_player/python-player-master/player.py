...
This er is designed to  music on a Raspberry Pi,
...
######################################################
#                                                    ############
print ("Starting Python Music er " + version + "." + revision )#
#                                                    ############
######################################################
def mkdir(directory):
...
def updater():
    ...
    if update == 0:
        ...
        try:
            ...
            if status == int(200):
                try:
                    filename = urllib.urlretrieve('http://' + url + '/python/downloads/er/music-er-' + str(ver) + '.tar.gz', 'music-er-' + str(ver) + '.tar.gz')
                except:
                    LogErr()
                    raise IOError
                bcast('Installing...')
                ...
                try:
                    mkdir('update')
                    os.rename("music-er-" + str(ver) + ".tar.gz", 'update/update.tar.gz')
                    os.chdir('update')
                    ...
                    log('Success!')
                    bcast('Done!')
                    bcast("Move 'er.py' from the folder 'update' to: " + os.path.dirname(os.getcwd()))
                except:
                    LogErr()
...
# To control the er remotely (non-functional)
...
def dis(text):
    ...
    pygame.dis.flip()
"""
def dis(text, background, screen):
        ...
        while i < len(sys.argv):
            ...
            if arg == "--console" or arg == "-c":
                ...
                try:
                    ...
                    LogErr()
                    print ("There was an error ing the file")
                    kill = True
##	    elif arg == "-h" or arg == "--help":
##                print ('s music in the "Music" folder within the current directory\n')
##                print ("Usage: " + sys.argv[0] + " [-hvc] [-f <filepath>]")
##                print ("Options: ")
##                print ("\t -h, --help\t Diss this help text")
##                print ("\t -v, --verbose\t Diss extra information")
##                print ("\t -c, --console\t Disables Pygame screen (text-only mode)")
##                print ("\t -f, --file\t s the file at the filepath specified")
...
try:
    ...
    if float(ver) > float(version):
        log('Update found!')
        bcast("Python Music er " + ver + " is availible")
...
try:
    if console == False:
        screen = pygame.dis.set_mode((1000, 200))
        pygame.dis.set_caption("Music er")
        background = pygame.Surface(screen.get_size())
        background = background.convert()
        background.fill((250, 250, 250))
except:
    bcast("Dis error, console mode active", True)
    log("Dis error")
    LogErr()
    console = True
log("er started")
...
#  the music
    while i != amount:
        ...
        if current not in ed:
# Try to load the track
            bcast("Now ing: " + current + " (" + str(songNum) + " out of " + str(amount) + ")")
            ...
            try:
                ...
                bcast("Couldn't  " + current)
#######################################################
#  loaded track
            pygame.mixer.music()
# Take user input for controlling er
            while pygame.mixer.music.get_busy():
                if console == False:
                    ...
                    t.start()
#######################################################
            if not current in ed:
                ed.append(current)
                i = i + 1
            sleep(0.2)
            songNum = songNum + 1
    bcast("All songs have been ed!")
    log('All songs have been ed')
    shutdown()
#######################################################
except:
    LogErr()
